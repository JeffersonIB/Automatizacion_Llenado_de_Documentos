...
namespace RRHH5.Pages.Documentos
{
    public partial class Genera_Contrato : System.Web.UI.Page
    {
        ...
        //void DDLCargarCosciedad()
        //{
        //    try
        //    {
        //        SqlCommand cmd = new SqlCommand("SP_CN_RCH00700", con);
        //        cmd.CommandType = System.Data.CommandType.StoredProcedure;
        //        con.Open();
        //        DDLSociedad.DataSource = cmd.ExecuteReader();
        //        DDLSociedad.DataTextField = "Sociedad";
        //        DDLSociedad.DataValueField = "Descripcion";
        //        DDLSociedad.DataBind();
        //        con.Close();
        //        //DDLSociedad.Items.Insert(0, new ListItem("--Seleccionar--", "0"));
        //    }
        //    catch (Exception)
        //    {
        //        throw;
        ...
        {
            // Aquí puedes realizar tareas finales antes de que se renderice la página
        }
        ...
        protected void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            // Realiza una consulta para obtener la información del empleado desde la base de datos
                ...
                if (employeeReader.Read())
                {
                    ...
                    if (representativeReader.Read())
                    {
                        ...
                        if (horariosReader.Read())
                        {
                            ...
                            // Parrafo Sociedad 
                            var objParrfafo0 = doc.Content.Paragraphs.Add(Type.Missing);
                            objParrfafo0.Range.Font.Size = 11;                                                                                                          // Tamaño de letra
                            objParrfafo0.Range.Font.Name = "Arial";                                                                                                     // Tipo de letra
                            objParrfafo0.Range.Font.Bold = 1;                                                                                                           // Negrita
                            objParrfafo0.Range.Font.Underline = Microsoft.Office.Interop.Word.WdUnderline.wdUnderlineSingle;                                            // Agregar subrayado
                            objParrfafo0.Range.ParagraphFormat.Alignment = Microsoft.Office.Interop.Word.WdParagraphAlignment.wdAlignParagraphCenter;                                                // Alineado justificado 
                            objParrfafo0.Range.Text = "CONTRATO INDIVIDUAL DE TRABAJO SUSCRITO ENTRE\nEXPORTADORA ENLASA, SOCIEDAD ANONIMA" + "\nY EL TRABAJADOR" + " " + nombre_empleado + "\n";   // Texto
                            objParrfafo0.Range.InsertParagraphAfter();
                            if (DateTime.TryParse(fecharelacion, out DateTime fechaSeleccionadarelacion))
                            {
                                ...
                                if (DateTime.TryParse(fechacontrato, out DateTime fechaSeleccionadacontrato))
                                {
                                    ...
                                    //Primer parrafo
                                    var objParrfafo1 = doc.Content.Paragraphs.Add(Type.Missing);
                                    objParrfafo1.Range.Font.Size = 11;                                                                  // Tamaño de letra   
                                    objParrfafo1.Range.Font.Name = "Arial";                                                             // Tipo de letra
                                    /*objParrfafo1.Range.Font.Bold = 1;*/                                                               // Negrita
                                    /*objParrfafo1.Range.Font.Underline = Microsoft.Office.Interop.Word.WdUnderline.wdUnderlineSingle;*/// Agregar subrayado
                                    objParrfafo0.Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphJustify;        // Alineado justificado  
                                    objParrfafo1.Range.Text =
                            ...
                            {
                                // Manejo de error si la conversión de la fecha falla
                            }

                            //Tercer Parrrafo
                            var objParrfafo3 = doc.Content.Paragraphs.Add(Type.Missing);
                            objParrfafo3.Range.Font.Size = 11;                                                              // Tamaño letra 
                            objParrfafo3.Range.Font.Name = "Arial";                                                         // Tipo letra
                            objParrfafo3.Range.Font.Bold = 1;                                                               // Negrita
                                                                                                                            //objParrfafo3.Range.Font.Underline = Microsoft.Office.Interop.Word.WdUnderline.wdUnderlineSingle;// Agregar subrayado
                            objParrfafo3.Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphJustify;    // Alineado justificado
                            objParrfafo3.Range.Text = "\r\n\r\n(F) __________________________\t\t\t(F) __________________________\r\n\t\tPATRONO\t\t\t\t\t\tTRABAJADOR\r\n";                                                        // Texto
                            objParrfafo3.Range.InsertParagraphAfter();


                            // Tu código para crear el contenido del documento
                            string tempFilePath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), fileName + ".docx");
                            doc.SaveAs(tempFilePath);
                            doc.Close();
                            wordApp.Quit();

                            Application wordAppPageCount = new Application();
                            Document docPageCount = wordAppPageCount.Documents.Open(tempFilePath);

                            int totalPages = docPageCount.ComputeStatistics(WdStatistic.wdStatisticPages);

                            // Verificar que totalPages sea válido antes de iterar
                            if (totalPages > 0)
                            {
                                // Itera a través de las páginas para insertar números de página
                                for (int pageNumber = 1; pageNumber <= totalPages; pageNumber++)
                                {
                                    // Mueve el cursor al pie de página
                                    if (docPageCount.Sections.Count >= pageNumber)
                                    {
                                        Section section = docPageCount.Sections[pageNumber];
                                        Range footerRange = section.Footers[WdHeaderFooterIndex.wdHeaderFooterPrimary].Range;

                                        // Inserta el número de página aumentando en 1 en cada página
                                        footerRange.InsertBefore("Página " + pageNumber + " de " + totalPages + " - ");
                                    }
                                }

                                docPageCount.Fields.Update();
                            }
                            else
                            {
                                // Manejar el caso en el que no hay páginas en el documento
                            }

                            docPageCount.Fields.Update();

                            string numberedFilePath = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "Numbered_" + fileName + ".docx");
                            docPageCount.SaveAs(numberedFilePath);
                            docPageCount.Close();

                            Response.Clear();
                            Response.ContentType = "application/vnd.openxmlformats-officedocument.wordprocessingml.document";
                            Response.AddHeader("Content-Disposition", "attachment; filename=" + fileName + ".docx");
                            Response.WriteFile(numberedFilePath);
                            Response.Flush();
                            Response.End();
