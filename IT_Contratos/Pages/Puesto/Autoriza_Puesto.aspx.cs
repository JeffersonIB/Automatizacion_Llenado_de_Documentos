...
{
    public partial class Autoriza_Puesto : System.Web.UI.Page
    {
        ...
        {
            if (AutorizacionRealizada_a())
            {
                ClientScript.RegisterClientScriptBlock(this.GetType(), "alert",
                 "swal('Error!', 'Función ya autorizada!', 'error')", true);
            }
            else
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("SP_AC_RCH00401a", con);
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.Parameters.Add("@IdProceso", System.Data.SqlDbType.VarChar).Value = DDLProceso.SelectedValue.ToString();
                    cmd.Parameters.Add("@IdPuesto", System.Data.SqlDbType.VarChar).Value = DDLPuesto.SelectedValue.ToString();
                    con.Open();
        ...
        {
            if (AutorizacionRealizada_l())
            {
                ClientScript.RegisterClientScriptBlock(this.GetType(), "alert",
                 "swal('Error!', 'Ya ha sido autorizada!', 'error')", true);
            }
            else
            {
                try
        ...
        protected void btnAutorizar_n_Click(object sender, EventArgs e)
        {
            if (AutorizacionRealizada_n())
            {
                ClientScript.RegisterClientScriptBlock(this.GetType(), "alert",
                 "swal('Error!', 'Función ya autorizada!', 'error')", true);
            }
